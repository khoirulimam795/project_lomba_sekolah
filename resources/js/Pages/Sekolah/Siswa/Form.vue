...
const form = useForm({
    ...
    surat_kesehatan: null, // File object kalau pilih baru
});

const onFile = (e) => {
    form.surat_kesehatan = e.target.files[0] || null;
};
...
const submit = () => {
    // FormData (multipart) HANYA kalau benar-benar ada file baru yang dipilih.
    // Kalau nggak ada file → kirim JSON biasa → paling aman untuk PUT/POST.
    const hasFile = form.surat_kesehatan instanceof File;

    // 👇 DIAGNOSA: kalau nanti masih aneh, lihat ini di Console saat klik simpan
    console.log('[SUBMIT]', {
    ...
    if (isEditing.value) {
        form.put(
            route('sekolah.siswa.update', { kontingen: props.kontingen.id, siswa: props.siswa.id }),
            opts
        );
    } else {
        form.post(
            route('sekolah.siswa.store', { kontingen: props.kontingen.id }),
            opts
        );
    }
};
...
<template>
    <SekolahLayout :header="isEditing ? 'Edit Siswa' : 'Tambah Siswa'">
        ...
        <div class="max-w-3xl mx-auto px-2 sm:px-0">
            <div class="mb-6">
                <Link :href="route('sekolah.siswa.index', { kontingen: kontingen.id })" class="text-sm text-forest hover:underline">
            ...
            <form @submit.prevent="submit" class="bg-white rounded-xl border border-line shadow-sm p-4 sm:p-6 space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.nisn" type="text" class="w-full border border-line rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-gold" placeholder="Opsional" />
                        <div v-if="form.errors.nisn" class="text-red-600 text-sm mt-1">{{ form.errors.nisn }}</div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block font-semibold mb-1">Golongan Pramuka</label>
                        <select v-model="form.golongan_pramuka" class="w-full border border-line rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-gold">
                            <option value="">Pilih</option>
                        ...
                        <div v-if="form.errors.golongan_pramuka" class="text-red-600 text-sm mt-1">{{ form.errors.golongan_pramuka }}</div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input v-model="form.nama_orang_tua" type="text" class="w-full border border-line rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-gold" placeholder="Opsional" />
                    </div>
                    <div>
                        <label class="block font-semibold mb-1">Jenjang Pendidikan</label>
                        <select v-model="form.jenjang_pendidikan" class="w-full border border-line rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-gold">
                            <option value="">Pilih</option>
                ...
                <div class="border-t border-line pt-5">
                    <label class="block font-semibold mb-1">📄 Surat Keterangan Sehat</label>
                    <p class="text-xs text-ink/50 mb-2">Foto/scan surat keterangan sehat. Format JPG/PNG/PDF, maks 2 MB. Opsional saat simpan, tapi Admin dapat meminta dokumen ini.</p>

                    <input
                        type="file"
                        accept=".jpg,.jpeg,.png,.pdf"
                        @change="onFile"
                        class="w-full text-sm text-ink/70 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-forest file:text-parchment file:font-semibold hover:file:bg-forest/90 file:cursor-pointer"
                    />
                    ...
                    <div v-if="newFileName" class="mt-3 flex items-center gap-2 text-sm bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                        <span>🆕</span><span class="truncate">File baru: <strong>{{ newFileName }}</strong></span>
                    </div>
                    ...
                    <div v-else-if="existingDoc" class="mt-3 bg-parchment/40 rounded-lg border border-line/60 p-3">
                        ...
                        <img v-if="isImage(existingDoc.mime)" :src="existingDoc.url" class="max-h-48 rounded-lg border border-line" alt="Surat sehat" />
                ...
                <div class="flex flex-col sm:flex-row items-center gap-3 pt-4 border-t border-line">
                    ...
                    <Link :href="route('sekolah.siswa.index', { kontingen: kontingen.id })" class="w-full sm:w-auto text-center px-5 py-2 border border-line rounded-lg hover:bg-parchment text-sm sm:text-base">
                        Batal
                    </Link>
                </div>
            </form>
