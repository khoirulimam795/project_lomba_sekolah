...
class LandingController extends Controller
{
    ...
    {
        $event = Event::where('status', 'aktif')->latest()->first() ?? Event::latest()->first();
    ...
    private function stats(Event $event): array
    {
        ...
        return [
            'pangkalan' => (int) \App\Models\Kontingen::where('event_id', $event->id)->distinct('team_id')->count('team_id'),
            'regu'      => (int) LombaKontingen::whereIn('lomba_id', $lombaIds)->count(),
            'lomba'     => (int) $lombaIds->count(),
            'juri'      => (int) \DB::table('lomba_juri')->whereIn('lomba_id', $lombaIds)->distinct('juri_id')->count('juri_id'),
        ];
    }
    ...
    {
        $now = Carbon::now();
        $mk = fn ($m, $s) => ['mulai' => $m, 'selesai' => $s, 'active' => $now->between(Carbon::parse($m)->startOfDay(), Carbon::parse($s)->endOfDay())];
        return [
            ['key' => 'daftar', 'label' => 'Pendaftaran', 'icon' => '📝'] + $mk($event->periode_pendaftaran_mulai, $event->periode_pendaftaran_selesai),
            ['key' => 'lomba', 'label' => 'Pelaksanaan Lomba', 'icon' => '🏕️'] + $mk($event->tanggal_pelaksanaan_mulai, $event->tanggal_pelaksanaan_selesai),
            ['key' => 'juara', 'label' => 'Pengumuman Juara', 'icon' => '🏆'] + ['mulai' => $event->tanggal_pelaksanaan_selesai, 'selesai' => $event->tanggal_pelaksanaan_selesai, 'active' => $now->gt(Carbon::parse($event->tanggal_pelaksanaan_selesai)->endOfDay())],
        ];
    }

    /** ✅ Rekap medali: filter golongan + ranking OLIMPIADE (tanpa poin) */
    private function standings(Event $event, ?string $selectedGolongan): array
    {
        $query = Juara::where('event_id', $event->id)->where('is_final', true)->with('kontingen.team');
        if ($selectedGolongan) {
    ...
    /** ✅ Hasil lomba: SEMUA alokasi siap tampil (nilai null = belum dinilai) + kirim kategori */
    private function leaderboards(Event $event): array
    {
        return $event->lombas()->orderBy('nama')->get()->map(function (Lomba $lomba) {
            $alokasis = LombaKontingen::where('lomba_id', $lomba->id)
                ->where('status', 'siap')
                ->with('kontingen.team')
                ->get()->keyBy('kontingen_id');

            $penilaianMap = Penilaian::where('lomba_id', $lomba->id)
                ->with(['kontingen.team', 'juri'])
                ->get()->groupBy('kontingen_id');

            $rows = $alokasis->map(function ($alokasi) use ($penilaianMap) {
                $kid = $alokasi->kontingen_id;
                $pn  = $penilaianMap[$kid] ?? collect();
                return [
                    'kontingen_id' => $kid,
                    'team_name'    => $alokasi->kontingen->team->name ?? '-',
                    'golongan'     => $alokasi->golongan ?? null,
                    'kategori'     => $alokasi->kategori ?? null,
                    'nilai_akhir'  => $pn->isNotEmpty() ? round((float) $pn->avg('nilai_akhir_juri'), 2) : null,
                    'jumlah_juri'  => $pn->count(),
                    'juri_scores'  => $pn->map(fn ($p) => ['nama' => $p->juri->name ?? '-', 'nilai' => $p->nilai_akhir_juri])->values()->all(),
                ];
            })->values()
              ->sortByDesc(fn ($r) => $r['nilai_akhir'] ?? -1)->values()
