   ...
   public function juaraUmum(Request $request)
{
    ...
    if ($selectedEventId) {
        ...
        // ✅ RANKING OLIMPIADE: tanpa bobot/poin
        $rows = $juaras->groupBy('kontingen_id')->map(function ($g) {
            ...
            return [
                ...
                'perak'        => $perak,
                'perunggu'     => $perunggu,
                // ❌ 'poin' DIHAPUS — tidak ada lagi perhitungan bobot
                'details'      => $g->map(fn ($j) => [
        ...
        foreach ($rows as $r) {
            $totalMedal['emas']     += $r['emas'];
            $totalMedal['perak']    += $r['perak'];
            $totalMedal['perunggu'] += $r['perunggu'];
        }

        $arr = $rows->values()->all();

        // ✅ SORT OLIMPIADE: emas DESC → perak DESC → perunggu DESC → kontingen_id ASC (tiebreaker stabil)
        usort($arr, fn ($a, $b) =>
    ...
    return inertia('Admin/Rekap/JuaraUmum', compact(
        'events', 'selectedEventId', 'ranking', 'totalMedal', 'lombaDinilai'
    ));
}
