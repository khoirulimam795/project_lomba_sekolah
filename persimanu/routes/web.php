...
Route::middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified'])->group(function () {
    Route::get('/dashboard', function (Request $request) {
        ...
        if ($user?->hasRole('juri'))             return redirect()->route('juri.dashboard');
        if ($user?->hasRole('operator-sekolah')) return redirect()->route('sekolah.pendaftaran.index');
...
Route::middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified', 'role:admin'])
    ->prefix('admin')->name('admin.')->group(function () {
        ...
        // ✅ Poin 4: duplikat SEBELUM resource (biar tidak ketangkap {lomba})
        Route::get('lombas/{lomba}/duplicate', [LombaController::class, 'duplicate'])->name('lombas.duplicate');
        Route::resource('lombas', LombaController::class)->except(['show']);

        // ❌ ROUTE KRITERIA DIHAPUS TOTAL (Poin 3) — pengaturan komponen kini menyatu di form Lomba
