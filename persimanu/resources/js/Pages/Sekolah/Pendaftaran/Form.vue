...
// ✅ Auto-fill dari pangkalan yang dikirim controller
const teamName = props.pangkalan?.name ?? props.kontingen?.team?.name ?? '';

const form = useForm({
    // ✅ WAJIB: event_id dikirim ke controller untuk validasi & penyimpanan
    ...
    <SekolahLayout :header="isEditing ? 'Edit Formulir Kesediaan' : 'Formulir Kesediaan (C.01)'">
        ...
        <div class="max-w-3xl mx-auto px-2 sm:px-0">
            ...
            <form @submit.prevent="submit" class="bg-white rounded-xl border border-line shadow-sm p-4 sm:p-6 space-y-6">
                ...
                <!-- ✅ HIDDEN FIELD: event_id wajib dikirim ke controller -->
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            <label class="block font-semibold mb-1">Nama Kontingen / Madrasah</label>
                            <input v-model="form.nama_kontingen" type="text" required
                        ...
                        <div class="sm:col-span-2">
                            <label class="block font-semibold mb-1">Nama Kepala Madrasah</label>
                            <input v-model="form.nama_kepala_madrasah" type="text" required
                                class="w-full border border-line rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-gold" />
                            <div v-if="form.errors.nama_kepala_madrasah" class="text-red-600 text-sm mt-1">{{ form.errors.nama_kepala_madrasah }}</div>
                        </div>
                        <div class="sm:col-span-2">
                            <label class="block font-semibold mb-1">Asal Instansi</label>
                            <input v-model="form.asal_instansi" type="text" required
                                class="w-full border border-line rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-gold" />
                            <p class="text-[11px] text-ink/45 mt-1">Otomatis dari pangkalan Anda — ubah hanya bila berbeda.</p>
                        ...
                        <div>
                            <label class="block font-semibold mb-1">No. HP / WA</label>
                            <input v-model="form.contact_phone" type="text" required
                                class="w-full border border-line rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-gold" placeholder="08xxxxxxxxxx" />
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block font-semibold mb-1">Pendamping Putra</label>
                            <div class="flex items-center gap-2">
                                <input v-model.number="form.pendamping_putra" type="number" min="0" max="20" required
                                    class="w-24 border border-line rounded-lg px-4 py-2 text-center font-mono font-bold text-lg focus:outline-none focus:ring-2 focus:ring-gold" />
                                <span class="text-sm text-ink/60">orang</span>
                            </div>
                            <div v-if="form.errors.pendamping_putra" class="text-red-600 text-sm mt-1">{{ form.errors.pendamping_putra }}</div>
                        </div>
                        <div>
                            <label class="block font-semibold mb-1">Pendamping Putri</label>
                            <div class="flex items-center gap-2">
                                <input v-model.number="form.pendamping_putri" type="number" min="0" max="20" required
                                    class="w-24 border border-line rounded-lg px-4 py-2 text-center font-mono font-bold text-lg focus:outline-none focus:ring-2 focus:ring-gold" />
                                <span class="text-sm text-ink/60">orang</span>
                            </div>
                            <div v-if="form.errors.pendamping_putri" class="text-red-600 text-sm mt-1">{{ form.errors.pendamping_putri }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block font-semibold mb-1">Peserta Putra</label>
                            <div class="flex items-center gap-2">
                                <input v-model.number="form.peserta_putra" type="number" min="0" max="50" required
                                    class="w-24 border border-line rounded-lg px-4 py-2 text-center font-mono font-bold text-lg focus:outline-none focus:ring-2 focus:ring-gold" />
                                <span class="text-sm text-ink/60">orang</span>
                            </div>
                            <div v-if="form.errors.peserta_putra" class="text-red-600 text-sm mt-1">{{ form.errors.peserta_putra }}</div>
                        </div>
                        <div>
                            <label class="block font-semibold mb-1">Peserta Putri</label>
                            <div class="flex items-center gap-2">
                                <input v-model.number="form.peserta_putri" type="number" min="0" max="50" required
                                    class="w-24 border border-line rounded-lg px-4 py-2 text-center font-mono font-bold text-lg focus:outline-none focus:ring-2 focus:ring-gold" />
                                <span class="text-sm text-ink/60">orang</span>
                            </div>
                            <div v-if="form.errors.peserta_putri" class="text-red-600 text-sm mt-1">{{ form.errors.peserta_putri }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-parchment/40 rounded-xl border border-line/60 p-4">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-sm">
                        <div class="text-ink/60">Pendamping:</div>
                        <div class="font-mono font-bold text-forest">{{ (form.pendamping_putra || 0) + (form.pendamping_putri || 0) }} orang ({{ form.pendamping_putra || 0 }} PA + {{ form.pendamping_putri || 0 }} PI)</div>
                        <div class="text-ink/60">Peserta:</div>
                        <div class="font-mono font-bold text-forest">{{ (form.peserta_putra || 0) + (form.peserta_putri || 0) }} orang ({{ form.peserta_putra || 0 }} PA + {{ form.peserta_putri || 0 }} PI)</div>
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row items-center gap-3 pt-4 border-t border-line">
                    <button type="submit" :disabled="form.processing"
                        ...
                        {{ isEditing ? 'Update Formulir Kesediaan' : 'Simpan Formulir Kesediaan' }}
                    </button>
