...
const submit = () => {
    // ✅ forceFormData HANYA kalau ada file baru → cegah field teks ke-drop (fix error simpan)
...
<template>
    <SekolahLayout :header="isEditing ? 'Edit Siswa' : 'Tambah Siswa'">
        ...
        <div class="max-w-3xl mx-auto px-2 sm:px-0">
            <div class="mb-6">
                <Link :href="route('sekolah.siswa.index', { kontingen: kontingen.id })" class="text-sm text-forest hover:underline">← Kembali ke daftar siswa</Link>
            </div>
    ...
    class="w-full border border-line rounded-lg px-4 py-2 bg-gray-100 text-ink/50 cursor-not-allowed">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select v-model="form.jenjang_pendidikan" class="w-full border border-line rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-gold">
                            <option value="">Pilih</option>
                            <option value="SD">SD</option><option value="MI">MI</option><option value="SMP">SMP</option>
                            <option value="MTs">MTs</option><option value="SMA">SMA</option><option value="MA">MA</option><option value="SMK">SMK</option>
                        </select>
                    </div>
                </div>
