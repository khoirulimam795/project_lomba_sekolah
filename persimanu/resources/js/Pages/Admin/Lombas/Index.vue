        ...
        <div class="px-2 sm:px-4 md:px-0">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-ink/60 mt-1">Kelola lomba per event.</p>
                </div>

                <Link
                    :href="route('admin.lombas.create')"
            ...
            <div class="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
                ...
                >
                    ...
                    <div class="p-3 sm:p-4 md:p-5 pb-2 sm:pb-3">
                        <div class="flex items-start justify-between gap-2 sm:gap-3">
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center gap-0.5 sm:gap-2 mt-0.5 sm:mt-1">
                                    <span class="text-[10px] sm:text-xs text-ink/50 truncate">
                                        {{ lomba.slug }}
                                    </span>
                                    <span class="text-[10px] sm:text-xs text-ink/40 hidden sm:inline">•</span>
                                    <span class="text-[10px] sm:text-xs text-ink/50 truncate">
                                        {{ lomba.event?.nama ?? '-' }}
                                    </span>
                                </div>
                            ...
                            <span
                                :class="[
                                    ...
                                    statusClass[lomba.status],
                                ]"
                            >
                                <span class="hidden xs:inline">{{ statusIcon[lomba.status] }}</span>
                                {{ statusLabel[lomba.status] }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="px-3 sm:px-4 md:px-5 pb-2 sm:pb-3">
                        <div class="text-[10px] sm:text-xs text-ink/60">
                            <div class="flex items-start gap-1 sm:gap-2">
                                <span class="mt-0.5">📝</span>
                                <span class="line-clamp-2 break-words">{{ truncateText(lomba.deskripsi, 70) }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="px-3 sm:px-4 md:px-5 pb-3 sm:pb-4 md:pb-5 pt-2 border-t border-line/50">
                        <div class="flex gap-1.5 sm:gap-2">
                            ...
                            <Link
                                :href="route('admin.lombas.edit', lomba.id)"
                    ...
                    >
                        ...
                        <div class="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-line px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between rounded-t-2xl">
                            <h3 class="font-display text-base sm:text-xl font-semibold text-forest flex items-center gap-2">
                                <span>🏆</span>
                                <span class="hidden xs:inline">Detail Lomba</span>
                        ...
                        <div v-if="selectedLomba" class="px-4 sm:px-6 py-4 sm:py-6 space-y-3 sm:space-y-5">
                            ...
                            <div class="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-2">
                                <span
                                    :class="[
                                        'px-3 sm:px-4 py-1 sm:py-1.5 rounded-full border text-xs sm:text-sm font-semibold self-start xs:self-auto',
                                        statusClass[selectedLomba.status],
                                    ]"
                                >
                                    {{ statusIcon[selectedLomba.status] }} {{ statusLabel[selectedLomba.status] }}
                                </span>
                            ...
                            <!-- Event -->
                            <div class="bg-parchment/30 rounded-xl p-3 sm:p-4 border border-line/50">
                                <label class="text-[10px] sm:text-xs font-semibold text-ink/50 uppercase tracking-wider">Event Terkait</label>
                        ...
                        <div class="sticky bottom-0 bg-white/95 backdrop-blur-sm border-t border-line px-4 sm:px-6 py-3 sm:py-4 flex flex-col xs:flex-row gap-2 xs:gap-3 rounded-b-2xl">
                            ...
                            <Link
                                v-if="selectedLomba"
                                :href="route('admin.lombas.edit', selectedLomba.id)"
