...
// ✅ Max total medali (bukan poin) untuk bar width
const maxTotal = computed(() =>
    props.ranking.length
        ? Math.max(...props.ranking.map((r) => (Number(r.emas) || 0) + (Number(r.perak) || 0) + (Number(r.perunggu) || 0)))
        : 1
);
...
// ✅ Lebar segmen berdasarkan proporsi medali terhadap total medali pangkalan
const segWidth = (row, key) => {
    const total = (row.emas || 0) + (row.perak || 0) + (row.perunggu || 0);
    return total ? (row[key] / total) * 100 : 0;
};

// ✅ Total medali per pangkalan (untuk bar width)
        ...
        <div class="relative overflow-hidden">
            ...
            <div class="relative px-3 sm:px-4 md:px-6 lg:px-0 pt-6 pb-12 space-y-6">
                ...
                <header class="space-y-2 sm:space-y-3">
                    <span class="inline-block text-[10px] sm:text-xs font-display font-bold tracking-[0.2em] sm:tracking-[0.25em] uppercase text-gold">
                        Modul 7 • Puncak Klasemen
                    </span>
                    <h2 class="font-display text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold text-forest leading-none">
                        Juara Umum
                    </h2>
                    <p class="text-xs sm:text-sm text-ink/60 max-w-2xl">
                        Klasemen akhir pangkalan lintas seluruh lomba dalam satu event.
                        Ranking berdasarkan <strong class="text-gold">medali emas terbanyak</strong>,
                        disusul <strong class="text-slate-500">perak</strong>, lalu <strong class="text-khaki">perunggu</strong>.
                    </p>
                </header>
                ...
                <section v-if="!selectedEventId" class="space-y-4">
                    ...
                    <div v-if="events.length === 0" class="bg-white rounded-xl border border-line shadow-sm p-8 sm:p-10 text-center text-ink/50">
                        ...
                        <p class="text-sm sm:text-base">Belum ada event.</p>
                    </div>
                ...
                <section v-else class="space-y-5 sm:space-y-6">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                        <div class="bg-white rounded-xl border border-line shadow-sm p-2.5 sm:p-3 md:p-4">
                            ...
                            <div class="font-display text-xl sm:text-2xl md:text-3xl font-extrabold text-forest tabular-nums">{{ ranking.length }}</div>
                        </div>
                        <div class="bg-white rounded-xl border border-line shadow-sm p-2.5 sm:p-3 md:p-4">
                            <div class="text-[8px] sm:text-[10px] uppercase tracking-wider text-ink/45 font-semibold flex items-center gap-1">🥇 Emas</div>
                            <div class="font-display text-xl sm:text-2xl md:text-3xl font-extrabold text-gold tabular-nums">{{ totalMedal.emas }}</div>
                        </div>
                        <div class="bg-white rounded-xl border border-line shadow-sm p-2.5 sm:p-3 md:p-4">
                            <div class="text-[8px] sm:text-[10px] uppercase tracking-wider text-ink/45 font-semibold flex items-center gap-1">🥈 Perak</div>
                            <div class="font-display text-xl sm:text-2xl md:text-3xl font-extrabold text-slate-500 tabular-nums">{{ totalMedal.perak }}</div>
                        </div>
                        <div class="bg-white rounded-xl border border-line shadow-sm p-2.5 sm:p-3 md:p-4">
                            <div class="text-[8px] sm:text-[10px] uppercase tracking-wider text-ink/45 font-semibold flex items-center gap-1">🥉 Perunggu</div>
                            <div class="font-display text-xl sm:text-2xl md:text-3xl font-extrabold text-khaki tabular-nums">{{ totalMedal.perunggu }}</div>
                        </div>
                    </div>

                    <!-- kosong -->
                    <div v-if="ranking.length === 0" class="bg-white rounded-xl border border-line shadow-sm p-8 sm:p-10 text-center text-ink/50">
                        <div class="text-4xl sm:text-5xl mb-3">🏆</div>
                        <p class="font-semibold text-ink/60 text-sm sm:text-base">Belum ada juara yang dibekukan di event ini.</p>
                        <p class="text-[10px] sm:text-xs md:text-sm mt-1">Buka <strong>Rekap per lomba</strong>, lalu pencet <strong>🏆 Hitung & Simpan Juara</strong> di tiap lomba.</p>
                    </div>

                    <template v-else>
                        <!-- PODIUM JUARA UMUM -->
                        <div class="bg-white rounded-2xl border border-line shadow-sm p-4 sm:p-6 md:p-8">
                            <h3 class="font-display text-base sm:text-lg md:text-xl font-extrabold text-forest mb-4 sm:mb-6 text-center">🏆 Podium Juara Umum</h3>
                                ...
                                >
                                    <template v-if="ranking[rank - 1]">
                                        ...
                                        <div class="text-center mb-2 sm:mb-3 px-1">
                                            <div class="font-display font-bold text-forest text-xs sm:text-sm md:text-base leading-tight line-clamp-2 max-w-[80px] sm:max-w-[120px] md:max-w-none">{{ ranking[rank - 1].team_name }}</div>
                                            <div class="font-mono font-extrabold text-lg sm:text-xl md:text-2xl mt-0.5 sm:mt-1 tabular-nums text-forest">
                                                {{ rowTotal(ranking[rank - 1]) }}
                                                <span class="text-[8px] sm:text-[10px] font-sans font-semibold text-ink/40">medali</span>
                                            </div>
                        ...
                        <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                            <div class="px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-b border-line bg-parchment/30 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                <h3 class="font-display text-base sm:text-lg md:text-xl font-extrabold text-forest">Klasemen Lengkap</h3>
                                <span class="text-[10px] sm:text-xs md:text-sm text-ink/50">{{ lombaDinilai }} lomba dinilai</span>
                            </div>
